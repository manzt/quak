...
function formatDataTypeName(type) {
	...
	return type.toString().toLowerCase().replace("<second>", "[s]").replace(
		"<millisecond>",
		"[ms]",
	).replace("<microsecond>", "[\xB5s]").replace("<nanosecond>", "[ns]").replace(
		"<day>",
		"[day]",
...
function formatterForDataTypeValue(type) {
	...
	if (
		arrow.DataType.isBinary(type) || arrow.DataType.isFixedSizeBinary(type) ||
	...
	if (arrow.DataType.isTime(type)) {
		return fmt(type.TValue, (ms) => {
			return instantFromTimeUnit(ms, type.unit).toZonedDateTimeISO("UTC")
...
var Histogram = class extends mc.MosaicClient {
	...
	channelField(channel, { exact = false } = {}) {
		...
		let c = exact
			...
			: this.#channels.find((c2) => c2.channel.startsWith(channel));
...
var DataTable = class extends mc2.MosaicClient {
	...
	constructor(source) {
		...
		root.appendChild(
			html.fragment`<table class="quak" style=${{
				tableLayout: "fixed",
			}}>${this.#thead}${this.#tbody}</table>`,
		);
	 ...
	 */
	query(filter = []) {
		return msql2.Query.from(this.#source.table).select(this.#columns).where(
			filter,
		).orderby(
			this.#orderby.filter((o) => o.order !== "unset").map((o) =>
				o.order === "asc" ? asc(o.field) : msql2.desc(o.field)
			),
	...
	fieldInfo(infos) {
		...
		this.#templateRow = html`<tr><td></td>${
			infos.map((info) =>
				html.fragment`<td class=${classes[info.column]}></td>`
			)
		}
			<td style=${{
			width: "99%",
...
function thcol(field, minWidth, vis) {
	...
	let sortButton =
		html`<span aria-role="button" class="sort-button" onmousedown=${nextSortState}>${svg}</span>`;
	let th = html`<th title=${field.name}>
		...
		fontSize: "12px",
		userSelect: "none",
	}}>${formatDataTypeName(field.type)}</span>
		${vis?.plot?.node()}
	</th>`;
	signals.effect(() => {
		...
		let element =
			{ "asc": uparrow, "desc": downarrow, "unset": null }[sortState.value];
...
function classof(schema) {
	...
	for (const field of schema.fields) {
		if (
			arrow2.DataType.isInt(field.type) || arrow2.DataType.isFloat(field.type)
		) {
			classes[field.name] = "number";
		}
		if (
			arrow2.DataType.isDate(field.type) ||
			arrow2.DataType.isTimestamp(field.type)
		) {
	...
	return {
		async initialize({ model }) {
			...
			let empty = await coordinator.query(
				msql3.Query.from(model.get("_table_name")).select(
					...model.get("_columns"),
				).limit(0).toString(),
			);
			schema = empty.schema;
